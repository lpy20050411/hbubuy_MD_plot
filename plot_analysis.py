...
def plot_rmsd(ax, complex_name):
    # 获取RMSD文件
    rmsd_files = [f for f in os.listdir('.') if f.startswith('RMSD_') and f.endswith('.xvg')]
    
    for rmsd_file in rmsd_files:
        # 提取文件名中的类型
        file_type = rmsd_file.replace('RMSD_', '').replace('.xvg', '')
        color = color_map.get(file_type, 'black')
...
def plot_rmsf(ax, complex_name):
    # 获取RMSF文件
    rmsf_files = [f for f in os.listdir('.') if f.startswith('RMSF_') and f.endswith('.xvg')]
    
    for rmsf_file in rmsf_files:
        # 提取文件名中的类型
        file_type = rmsf_file.replace('RMSF_', '').replace('.xvg', '')
        color = color_map.get(file_type, 'black')
...
def plot_rog(ax, complex_name):
    ...
    # 绘制曲线
    ax.plot(time, rog, label='RoG', color='blue')
